...
- `Sidebar` can now answer availability for addon IDs, expose addon IDs plus render/search metadata, and keep categories visible when addon-only entries are the only visible entries. Addon-only rows can be visible and hoverable, but dragging/instantiating sidebar rows into the live graph still depends on built-in `NodeType` or custom-node entries.
- Saved-graph validation can report unsupported persisted addon IDs, registered addon-only saved IDs can reload as unsupported placeholders, registry definitions can expose addon mode metadata, and registered addon executors can route by preserved placeholder type ID, but Pathmind still does not instantiate addon nodes from sidebar/search or validate addon-specific sockets, parameters, selected modes, or richer execution behavior.
