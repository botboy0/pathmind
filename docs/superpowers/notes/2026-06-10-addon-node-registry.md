...
- Added focused tests for built-in lookup, direct addon registration, and entrypoint-style addon registration.
...
This slice intentionally does not update editor/sidebar enumeration or execution dispatch. Those will move to the registry in later slices after this API foundation is in place.
...
- The registry currently captures metadata only; addon node creation, editor listing, parameter metadata, traits, and execution routing are still enum-backed in later call sites.
