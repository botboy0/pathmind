...
- Saved-graph validation can report unsupported persisted addon IDs, registered addon-only saved IDs can reload as unsupported placeholders, and registry definitions can expose addon mode metadata, but Pathmind still does not instantiate executable addon nodes or validate addon-specific sockets, parameters, selected modes, or execution behavior.
