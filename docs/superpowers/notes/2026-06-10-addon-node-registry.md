...
- Saved-graph validation can report unsupported persisted addon IDs, and registered addon-only saved IDs can reload as unsupported placeholders, but Pathmind still does not instantiate executable addon nodes or validate addon-specific sockets, parameters, modes, or execution behavior.
