...
- **Check before you touch:** `git diff upstream/main main --numstat -- <file>` tells you whether a file is upstream code (unchanged/small delta) or fork-owned (all-insertions). For upstream files, keep any touch minimal and behavior-preserving; if a real behavior change in upstream code seems unavoidable, stop and discuss it with Philip (the fork maintainer — he coordinates with the upstream developer directly) instead of implementing it.
