...
- [ ] mc-testkit: replace the in-container Pathmind Gradle build with a prebuilt-jar loadout — ship a minimal Fabric dev-shell workspace in the image and deliver **both** jars (pathmind-fabric + addon) via the loadout overlay, so spec runs no longer depend on the WSL clone (`~/mc-testkit/pathmind`) being synced and the per-run `gradlew runClient` build disappears.
